...
echo "<tr><th>Order Number / <br/>
      Customer / <br/>
	  Order Date</td>
	  <th>Product(s) Ordered</td>
	  <th>Total<br/>Amount<br/>(Rs.)</td>
      <th>Shipping Address</td><th>Action to be<br/>performed</td>";
echo "</tr>";
if(!$con)
{
...
$res = mysql_query("SELECT o.order_id, c.custname, o.order_date, o.addrline1,	o.addrline2, o.city, o.state, o.pin, o.phone, o.emailid, o.total FROM ORDER_MASTER o, CUSTOMER_MASTER c WHERE o.customer = c.custid AND o.ord_status = 'order in process'");
while($row = mysql_fetch_array($res))
{
  ...
  echo "<td>".
		...
		"</td>";
  echo "<td>";
  $res2 = mysql_query("SELECT * FROM ORDER_DETAIL o, PRODUCT_MASTER p WHERE o.product = p.prodid AND o.quantity > p.pstock AND o.order = '".$row['order_id']."'");
  if($row2 = mysql_fetch_array($res2))
  { 
    echo "<button type=\"button\" disabled=\"disabled\">Not enough stock to deliver the order</button><br/>";
  }
  else
  {
    echo "<form name='deliver' method='post' action='deliver_order.php'>";
	echo "<input type='hidden' name='order' value='".$row['order_id']."'>";
	echo "<button type='submit'>Deliver the order</button></form><br/>";
  }
  echo "<form name='reject' method='post' action='reject_order.php'>";
  echo "<input type='hidden' name='order' value='".$row['order_id']."'>";  
  echo "<button type='submit'>Cancel the order</button></form></td>";
  echo "</tr>";
}
echo "</table>";
?>
